...
/**
 * История разговора для модели.
 *
 * Текст сообщения лежит в content, а не отдельной колонкой: у
 * сообщения бывают файлы, кнопки и реакции, и «тело» у него одно —
 * разобранное содержимое. Запрос за несуществующей колонкой стоил
 * этому месту молчания: ручка черновика отвечала пятисоткой, а
 * автоответ молчал, и оба выглядели как «ИИ не работает».
 *
 * Текст запроса живёт здесь, а не в каждой из двух ручек: они обязаны
 * брать одну и ту же историю, и разойтись им нельзя.
 *
 * Порядок по sent_at: это время у провайдера, а не время записи у нас,
 * и по нему же лежит индекс разговора.
 */
export const AI_HISTORY_SQL = `SELECT direction, content->>'text' AS body
   FROM messages
  WHERE conversation_id = $1
    AND content->>'text' IS NOT NULL AND content->>'text' <> ''
