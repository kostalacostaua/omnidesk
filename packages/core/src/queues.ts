...
export interface WaLoginState {
   ...
   */
  /*
   * linking — код уже приняли, идёт обмен ключами. Отдельное состояние,
   * потому что QR в этот момент показывать нельзя: он отработал, а
   * телефон ждёт и пишет «виконується вхід».
   */
  state: 'starting' | 'qr' | 'linking' | 'done' | 'error';
