...
export type ChannelType =
  ...
  | 'viber_business'
  /**
   * WhatsApp через шлюз: посредник держит сессию WhatsApp Web, клиент
   * подключает свой обычный номер по QR. Не официальный API и не
   * заменяет его — он для тех, у кого номер уже живёт годами и
   * переносить его в Business Manager никто не будет.
   */
  | 'whatsapp_user'
  /**
...
export function computeResponseWindow(
  ...
  switch (channelType) {
    ...
    case 'telegram_bot':
    case 'telegram_user':
    // У шлюза окна нет: на той стороне обычный WhatsApp Web, и он не
    // знает про сутки — это правило официального API, а не мессенджера.
    // Писать первым технически можно; вопрос не в окне, а в том, что
    // за такое блокируют номер, и говорится об этом в другом месте.
    case 'whatsapp_user':
