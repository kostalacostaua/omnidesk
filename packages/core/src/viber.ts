 ...
 *
 * Здесь только бизнес-канал: Viber Business Messages с именем
 * отправителя вместо номера, через официального партнёра.
 *
 * Номерной Viber — отдельная история и в интерфейсе помечен как «в
 * работе». Своего API для личных аккаунтов у Viber нет; то, что
 * продают агрегаторы, — эмуляция десктопного клиента, подключённого
 * по QR, и номер за коммерческое использование могут заблокировать.
 * Если канал появится, он придёт готовым потоком от партнёра, который
 * это уже держит, а не отдельным протоколом внутри нас. Поэтому
 * `normalizeViber` и очередь входящих ничего не знают про способ
 * доставки: подключить второй источник — значит добавить тип канала,
 * а не переписывать ленту.
 *
