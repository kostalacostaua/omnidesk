 ...
 *
 * Про «номерной Viber». Его нет и быть не может: у Viber нет открытого
 * протокола для личных аккаунтов, а библиотеки, которые притворяются
 * телефоном, нарушают их правила — номер за это блокируют. Поэтому
 * здесь то, что в Viber действительно работает для переписки с
 * клиентами: Viber Business Messages с именем отправителя вместо
 * номера, подключённые через официального партнёра.
 *
