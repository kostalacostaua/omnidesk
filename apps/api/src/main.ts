...
const app = Fastify({
  ...
  trustProxy: true,
  /*
   * Журнал на каждый запрос выключен.
   *
   * Открытая страница опрашивает сервер раз в три секунды, и на каждый
   * опрос уходило две строки журнала. У десятка открытых вкладок это
   * тысячи строк в час, в которых тонет единственная нужная — про
   * отказ. Хостинг вдобавок обрезает поток и выбрасывает как раз её.
   *
   * Вместо этого ниже стоит правило: записываем только то, что
   * отвечает ошибкой или отвечает долго.
   */
 ...
 *
 * Только отказы и медленные ответы. Успешный опрос списка диалогов не
 * рассказывает ничего, а происходит он у каждой открытой страницы
 * двадцать раз в минуту; из-за него в журнале не найти ни одного
 * настоящего отказа.
 *
 * Секунда — граница не из головы: столько человек ждёт, не замечая
 * ожидания. Всё, что дольше, однажды станет жалобой «тормозит», и к
 * этому моменту записи должны быть.
 */
const SLOW_MS = 1000;
...
app.addHook('onResponse', async (req, reply) => {
  ...
  if (reply.statusCode >= 500) app.log.error(line, 'Запрос отвечен ошибкой');
