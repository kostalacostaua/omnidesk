...
/**
 * С какого домена-витрины открывать оплату.
 *
 * Из списка берём тот, что с www, а не короткий домен. Короткий у
 * регистраторов чаще всего не обслуживается сервисом, а стоит
 * переадресацией на www — и переадресация выбрасывает путь: человек
 * нажимал «оплатити», а попадал на главную страницу витрины без
 * всякого окна оплаты, и по адресу это не отличить от поломки оплаты.
 * Раз www в списке, он обслуживается.
 */
export function payHost(hosts: string[]): string {
  return hosts.filter((h) => h.startsWith('www.'))[0] ?? hosts[0] ?? '';
}
