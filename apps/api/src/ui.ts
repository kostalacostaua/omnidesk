...
function tabProfile(){
  api('/me').then(function(d){
    ...
    el('langSel').onchange = function(){
      langSet(this.value);
      /* Запоминаем и на сервере: письма о сроке и оплате пишутся тогда,
         когда человека нет на экране, и язык браузера им недоступен. */
      api('/me', { method:'PATCH', body:{ lang: this.value } }).catch(function(){});
