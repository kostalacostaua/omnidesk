 ...
 *
 * Отдельная проверка, а не ещё один префикс в списке администраторских:
 * там решается, что можно администратору клиента, а здесь — что видит
 * владелец сервиса. Смешивать их нельзя: администратор клиента не
 * должен получить список всех организаций ни при каком стечении
 * обстоятельств.
 *
 * Сравнение по границе пути: /administrative — чужой путь, и совпадать
 * с панелью он не должен.
 */
